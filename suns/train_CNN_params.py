...
from models.shallow_unet.shallow_unet_original_imp.suns.Network.data_gen import data_gen
from models.shallow_unet.shallow_unet_original_imp.suns.Network.shallow_unet import (
    get_shallow_unet,
)
from models.shallow_unet.shallow_unet_original_imp.suns.Network.par2 import fastuint
...
def train_CNN(
    ...
    for cnt, Exp_ID in enumerate(list_Exp_ID_train):
        ...
        if num_frame >= num_train_per:
            train_imgs[
                cnt * num_train_per : (cnt + 1) * num_train_per, :, :
            ] = np.array(
                h5_img["network_input"][
                    start_frame_train : train_every * num_train_per : train_every
                ]
            )
            train_masks[
                cnt * num_train_per : (cnt + 1) * num_train_per, :, :
            ] = np.array(
                h5_mask["temporal_masks"][
                    start_frame_train : train_every * num_train_per : train_every
                ]
            )
        else:
    ...
    if list_Exp_ID_val is not None:
        ...
        for cnt, Exp_ID in enumerate(list_Exp_ID_val):
            h5_img = h5py.File(os.path.join(dir_img, Exp_ID + ".h5"), "r")
            val_imgs[cnt * num_val_per : (cnt + 1) * num_val_per, :, :] = np.array(
                h5_img["network_input"][
                    start_frame_val : val_every * num_val_per : val_every
                ]
            )
            h5_img.close()
            h5_mask = h5py.File(os.path.join(dir_mask, Exp_ID + ".h5"), "r")
            val_masks[cnt * num_val_per : (cnt + 1) * num_val_per, :, :] = np.array(
                h5_mask["temporal_masks"][
                    start_frame_val : val_every * num_val_per : val_every
                ]
            )
    ...
    train_gen = data_gen(
        train_imgs, train_masks, batch_size=BATCH_SIZE, flips=True, rotate=True
    )
    if list_Exp_ID_val is not None:
        val_gen = data_gen(
            val_imgs, val_masks, batch_size=BATCH_SIZE, flips=False, rotate=False
        )
...
def parameter_optimization_pipeline(
    ...
    list_Recall, list_Precision, list_F1 = parameter_optimization(
        pmaps, Params_set, filename_GT, useMP=useMP, useWT=useWT, p=p
    )
    return list_Recall, list_Precision, list_F1


def parameter_optimization_cross_validation(
    cross_validation,
    list_Exp_ID,
    Params_set,
    dims,
    ...
    for eid, Exp_ID in enumerate(list_Exp_ID):
        if max_eid is not None:
            if eid > max_eid:
                continue
        gc.collect()
        list_saved_results = glob.glob(
            os.path.join(
                dir_temp, "Parameter Optimization CV* Exp{}.mat".format(Exp_ID)
            )
        )
        saved_results_CVall = os.path.join(
            dir_temp, "Parameter Optimization CV{} Exp{}.mat".format(nvideo, Exp_ID)
        )
        ...
        for CV in list_CV:
            mat_filename = os.path.join(
                dir_temp, "Parameter Optimization CV{} Exp{}.mat".format(CV, Exp_ID)
            )
            if os.path.exists(mat_filename) and load_exist:
                ...
                Table = np.vstack(
                    [
                        array_minArea.ravel(),
                        array_AvgArea.ravel(),
                        array_thresh_pmap.ravel(),
                        array_cons.ravel(),
                        array_thresh_COM.ravel(),
                        array_thresh_IOU.ravel(),
                        list_Recall.ravel(),
                        list_Precision.ravel(),
                        list_F1.ravel(),
                    ]
                ).T
    ...
    for CV in list_CV:
        ...
        Info_dict = {
            ...
            "Params": Params,
            "Table": Table,
            "Recall_train": Recall_train[CV],
            "Precision_train": Precision_train[CV],
            "F1_train": F1_train[CV],
        }
        savemat(
            os.path.join(dir_output, "Optimization_Info_{}.mat".format(CV)), Info_dict
        )
