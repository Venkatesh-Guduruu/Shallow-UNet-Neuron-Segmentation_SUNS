...
from models.shallow_unet.shallow_unet_original_imp.suns.PreProcessing.par1 import (
    fastexp,
    fastmask,
    fastlog,
    fastconv,
...
def find_dataset(h5_file):
    ...
    if num_dset == 1:
        ...
        if list_size[ind_dset] > 0:
            ...
            raise (
                KeyError(
                    "Cannot find any dataset. Please do not put the movie under any group"
                )
            )
    return dset
...
def plan_fft(frames_init, dims1, prealloc=True):
    ...
    if prealloc:
        bf = pyfftw.zeros_aligned(
            (frames_init, rows1, cols1 // 2 + 1), dtype="complex64", n=8
        )
    else:
        bf = pyfftw.empty_aligned(
            (frames_init, rows1, cols1 // 2 + 1), dtype="complex64", n=8
        )
...
def median_calculation(network_input, med_frame2=None, median_decimate=1, display=True):
    ...
    temp_noise = (med_frame2[:rows, :cols, 0] - med_frame2[:rows, :cols, 1]) / (
        math.sqrt(2) * special.erfinv(0.5)
    )
    ...
    med_frame3 = np.copy(
        med_frame2[:rows, :cols, :].transpose([2, 0, 1])
    )  # Using copy to avoid computer crashing
    if display:
...
def SNR_normalization(network_input, med_frame2=None, median_decimate=1, display=True):
    ...
    med_frame3 = median_calculation(
        network_input, med_frame2, median_decimate, display=False
    )
    if display:
        endmedtime = time.time()
        # print('median computation: {} s'.format(endmedtime - start))
...
def median_normalization(
    network_input, med_frame2=None, median_decimate=1, display=True
):
    ...
    fastnormback(
        network_input[:, :rows, :cols], max(1, med_frame2[:rows, :cols, 0].mean())
    )
    # med_frame3 = med_frame2.transpose([2,0,1])
    med_frame3 = np.copy(
        med_frame2.transpose([2, 0, 1])
    )  # Using copy to avoid computer crashing
    if display:
        endnormtime = time.time()
        # print('normalization: {} s'.format(endnormtime - endmedtime))
...
def preprocess_video(
    ...
    network_input, _ = preprocess_complete(
        bb,
        dimspad,
        dimsnb,
        network_input,
