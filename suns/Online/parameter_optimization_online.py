...
from models.shallow_unet.shallow_unet_original_imp.suns.Online.functions_online import (
    merge_2,
)
from models.shallow_unet.shallow_unet_original_imp.suns.PostProcessing.seperate_neurons import (
    watershed_neurons,
    separate_neuron,
)
from models.shallow_unet.shallow_unet_original_imp.suns.PostProcessing.combine import (
    unique_neurons2_simp,
    group_neurons,
    piece_neurons_IOU,
    piece_neurons_consume,
)
from models.shallow_unet.shallow_unet_original_imp.suns.PostProcessing.refine_cons import (
    refine_seperate_multi,
)
...
def merge_complete_nocons(uniques, times_uniques, dims, Params):
    ...
    if uniques.size:
        ...
        groupedneurons, times_groupedneurons = group_neurons(
            uniques, thresh_COM, thresh_mask, dims, times_uniques
        )
        # Merge neurons with high IoU.
        piecedneurons_1, times_piecedneurons_1 = piece_neurons_IOU(
            groupedneurons, thresh_mask, thresh_IOU, times_groupedneurons
        )
        # Merge neurons with high consume ratio.
        piecedneurons, times_piecedneurons = piece_neurons_consume(
            piecedneurons_1, avgArea, thresh_mask, thresh_consume, times_piecedneurons_1
        )
        # %% Final result
        ...
        if masks_final_2.size:
            ...
            Masks_2 = [
                (x >= x.max() * thresh_mask).astype("float") for x in masks_final_2
            ]
...
# %%
def optimize_combine_1_online(
    list_uniques: list,
    list_times_uniques: list,
    dims: tuple,
    Params: dict,
    filename_GT: str,
):
    ...
    tuple_temp = merge_complete_nocons(
        list_uniques[0], list_times_uniques[0], dims, Params
    )
    for ind in range(1, len(list_uniques)):
        # merge additional "merge_every" frames
        tuple_add = merge_complete_nocons(
            list_uniques[ind], list_times_uniques[ind], dims, Params
        )
    ...
    Recall_k, Precision_k, F1_k = refine_seperate_multi(
        GTMasks_2, masks_final_2, times_final, list_cons, thresh_mask, display=False
    )
    return Recall_k, Precision_k, F1_k


def optimize_combine_3_online(
    list_totalmasks,
    list_neuronstate,
    list_COMs,
    list_areas,
    ...
    for ind in range(len(list_areas)):
        ...
        uniques, times_uniques = unique_neurons2_simp(
            list_totalmasks[ind],
            list_neuronstate[ind],
            list_COMs[ind],
            list_areas[ind],
            list_probmapID[ind],
            minArea,
    ...
    if not times_uniques:
        ...
        if useMP:
            p3 = mp.Pool()
            list_temp = p3.starmap(
                optimize_combine_1_online,
                [
                    (
                        list_uniques,
                        list_times_uniques,
                    ...
                    for thresh_COM in list_thresh_COM
                    for thresh_IOU in list_thresh_IOU
                ],
                chunksize=1,
            )
            list_Recall_inter = np.vstack([x[0] for x in list_temp]).reshape(size_inter)
            list_Precision_inter = np.vstack([x[1] for x in list_temp]).reshape(
                size_inter
            )
            ...
            for j1, thresh_COM in enumerate(list_thresh_COM):
                for j2, thresh_IOU in enumerate(list_thresh_IOU):
                    (Recal_k, Precision_k, F1_k) = optimize_combine_1_online(
                        list_uniques,
                        list_times_uniques,
                        dims,
                        {
...
# %%
def parameter_optimization_online(
    pmaps: np.ndarray,
    frames_initf,
    merge_every,
    Params_set: dict,
    ...
    for i3, thresh_pmap in enumerate(list_thresh_pmap):
        ...
        if useMP:
            segs = p.starmap(
                separate_neuron,
                [(frame, thresh_pmap, minArea, 0, False) for frame in pmaps],
                chunksize=1,
            )
        else:
            segs = [
                separate_neuron(frame, thresh_pmap, minArea, 0, False)
                for frame in pmaps
            ]
        ...
        for i2, avgArea in enumerate(list_avgArea):
            if useWT:  # Try to further segment large masks using watershed.
                ...
                if useMP:
                    segs2 = p.starmap(
                        watershed_neurons,
                        [((Lx, Ly), frame_seg, minArea, avgArea) for frame_seg in segs],
                        chunksize=32,
                    )
                else:
                    segs2 = [
                        watershed_neurons((Lx, Ly), frame_seg, minArea, avgArea)
            ...
            if num_neurons == 0:
                ...
                list_probmapID = [
                    np.hstack(
                        [
                            ind * np.ones(x[1].size, dtype="uint32")
                            for (ind, x) in enumerate(segs_init)
                        ]
                    )
                ]
                ...
                for t in range(frames_initf, len(segs2), merge_every):
                    ...
                    list_probmapID.append(
                        np.hstack(
                            [
                                ind * np.ones(x[1].size, dtype="uint32")
                                for (ind, x) in enumerate(segs_add)
                            ]
                        )
                    )
                ...
                if useMP:
                    try:
                        list_result = p.starmap(
                            optimize_combine_3_online,
                            [
                                (
                                    list_totalmasks,
                                    list_neuronstate,
                        ...
                        list_F1[:, i2, i3, :, :, :] = 0
                        print(
                            "OverflowError. Size of totalmasks is larger than 4 GB. thresh_pmap is likely too low."
                        )
                    except MemoryError:
                        list_Recall[:, i2, i3, :, :, :] = 0
                        list_Precision[:, i2, i3, :, :, :] = 0
                        list_F1[:, i2, i3, :, :, :] = 0
                        print(
                            "MemoryError. Too much memory is needed. thresh_pmap is likely too low."
                        )
