...
from models.shallow_unet.shallow_unet_original_imp.suns.Online.par_online import (
    fastlog_2,
    fastmask_2,
    fastexp_2,
    fastconv_2,
...
def preprocess_online(
    ...
    if useTF:  # Temporal filtering
        ...
        fastconv_2(
            past_frames[:, :rowspad, :colspad],
            frame_SNR[:rowspad, :colspad],
            Poisson_filt,
        )
    else:
    ...
    if useSNR:
        ...
        fastnormback_2(
            frame_SNR[:rowspad, :colspad],
            max(1, med_frame3[0, :colspad, :colspad].mean()),
        )
...
def separate_neuron_online(
    frame_prob, pmaps_b, thresh_pmap_float, minArea, avgArea, useWT=False
):
...
def merge_2(tuple1, tuple2, dims, Params):
    ...
    if (
        N2 == 0
    ):  # If no additional masks is found in the new frames, the output is tuple1
        return tuple1
    if area2.ndim == 0:
    ...
    if np.all(
        merged
    ):  # If all new masks are merged to old masks, the mask list does not change
...
def merge_2_nocons(tuple1, tuple2, dims, Params):
    ...
    # pairs of neuron to be merged
    merge = np.logical_or.reduce(
        [IOU >= thresh_IOU, consume_1 >= thresh_consume, consume_2 >= thresh_consume]
    )
    ...
    for xi, yi in zip(x, y):
        xi0 = ind_nocons[
            xi
        ]  # xi is the index in the "nocons" masks. xi0 is the index in the full tuple1.
...
def final_merge(tuple_temp, Params):
    ...
    masks_final_2, times_2 = piece_neurons_consume(
        masks_1, avgArea, thresh_mask, thresh_consume, times_1
    )
