...
def median_std(result, med_frame2):
    ...
    temp_noise = (med_frame2[:, :, 0] - med_frame2[:, :, 1]) / (
...
def SNR_normalization(
    ...
    if not update_baseline:
        ...
        for t_update in range(frames_initf, network_input.shape[0], frames_init):
            result = np.copy(
                network_input[
                    t_update : t_update + frames_init, :rows, :cols
                ].transpose([1, 2, 0])
            )
            fastnormf(
...
def median_normalization(
    ...
    if not update_baseline:
        ...
        fastnormback(
            network_input[: frames_initf + frames_init, :rows, :cols],
            max(1, med_frame2[:, :, 0].mean()),
        )
        for t_update in range(
            frames_initf + frames_init, network_input.shape[0], frames_init
        ):
            result = np.copy(
                network_input[
                    t_update - frames_init : t_update, :rows, :cols
                ].transpose([1, 2, 0])
            )
            fastquant(result, np.array([0.5], dtype="float32"), med_frame2[:, :, 0:1])
            fastnormback(
...
def preprocess_complete_online(
    bb,
    dimspad,
    network_input=None,
    med_frame2=None,
    ...
    if useTF:  # Temporal filtering
        temporal_filtering(
            bb[:, :rowspad, :colspad], network_input, Poisson_filt, display=display
        )
...
def preprocess_video_online(
    ...
    network_input = preprocess_complete_online(
        bb,
        (rowspad, colspad),
        network_input,
        med_frame2,
