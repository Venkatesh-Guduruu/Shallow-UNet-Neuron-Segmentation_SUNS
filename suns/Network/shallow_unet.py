...
def get_shallow_unet(size=None, Params_loss=None):
    ...
    c1 = tf.keras.layers.Conv2D(
        4, (3, 3), activation=activation, kernel_initializer="he_normal", padding="same"
    )(inputs)

    mp1 = tf.keras.layers.MaxPooling2D((2, 2))(c1)
    c2 = tf.keras.layers.Conv2D(
        8, (3, 3), activation=activation, kernel_initializer="he_normal", padding="same"
    )(mp1)

    mp2 = tf.keras.layers.MaxPooling2D((2, 2))(c2)
    ...
    ct2 = tf.keras.layers.Conv2DTranspose(8, (2, 2), strides=(2, 2), padding="same")(c3)
    ...
    ct1 = tf.keras.layers.Conv2DTranspose(4, (2, 2), strides=(2, 2), padding="same")(c2)
    ct1 = tf.keras.layers.concatenate([ct1, c1], axis=3)  # The first skip connection
    ...
    def loss_func(y_true, y_pred):
        if Params_loss is None:
            ...
            return total_loss(
                ...
                BCE=Params_loss["BCE"],
                FL=Params_loss["FL"],
                gamma=Params_loss["gamma"],
                alpha=Params_loss["alpha"],
            )
...
def get_shallow_unet_more(
    size=None, n_depth=3, n_channel=4, skip=[1], activation="elu", Params_loss=None
):
    ...
    if n_depth > 2:
        ...
        if n_depth > 3:
            ...
            ct3 = tf.keras.layers.Conv2DTranspose(
                4 * n_channel, (2, 2), strides=(2, 2), padding="same"
            )(do4)
            if 3 in skip:
        ...
        ct2 = tf.keras.layers.Conv2DTranspose(
            2 * n_channel, (2, 2), strides=(2, 2), padding="same"
        )(do3)
        if 2 in skip:
    ...
    ct1 = tf.keras.layers.Conv2DTranspose(
        n_channel, (2, 2), strides=(2, 2), padding="same"
    )(do2)
    if 1 in skip:
...
def get_shallow_unet_more_equal(
    size=None, n_depth=3, n_channel=4, skip=[1], activation="elu", Params_loss=None
):
    """Get a shallow U-Net model. This function has some options to vary the model,
        including the number of resolution depth ("n_depth"), number of channels per featrue map ("n_channel"),
