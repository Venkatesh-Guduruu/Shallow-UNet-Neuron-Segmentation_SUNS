...
from models.shallow_unet.shallow_unet_original_imp.suns.Online.functions_online import (
    ...
    preprocess_online,
    CNN_online,
    separate_neuron_online,
    refine_seperate_cons_online,
    final_merge,
)
from models.shallow_unet.shallow_unet_original_imp.suns.Online.functions_init import (
    init_online,
    plan_fft2,
)
from models.shallow_unet.shallow_unet_original_imp.suns.PreProcessing.preprocessing_functions import (
    preprocess_video,
    plan_fft,
    plan_mask2,
    load_wisdom_txt,
...
from models.shallow_unet.shallow_unet_original_imp.suns.Network.shallow_unet import (
    get_shallow_unet,
)
from models.shallow_unet.shallow_unet_original_imp.suns.PostProcessing.par3 import (
    fastthreshold,
)
...
from models.shallow_unet.shallow_unet_original_imp.suns.PostProcessing.complete_post import (
    complete_segment,
)
...
def suns_batch(
    ...
    video_input, start = preprocess_video(
        dir_video,
        Exp_ID,
        Params_pre,
        useSF=useSF,
    ...
    Params_post_copy[
        "thresh_pmap"
    ] = None  # Avoid repeated thresholding in postprocessing
    ...
    Masks_2, times_active = complete_segment(
        pmaps_b, Params_post_copy, display=display, p=p, useWT=useWT
    )
    if display:
        finish = time.time()
    ...
    return prob_map, Masks, Masks_2, times_active, time_total, time_frame
...
def suns_online(
    ...
    if useSF == True:
        ...
        # FFT planning
        (bb, bf, fft_object_b, fft_object_c) = plan_fft(
            frames_init, (rowsfft, colsfft), prealloc
        )
        if export:
    ...
    if update_baseline:  # Set the parameters for median update
        ...
        if (
            useSF
        ):  # If spatial filtering is used, the padded rows and columns are nonzero
            Lu2 = colsnb
    ...
    for t in range(frames_initf, nframesf):
        ...
        # PreProcessing
        if useTF:
            frame_SNR, frame_tf = preprocess_online(
                bb,
                dimspad,
                dimsnb,
                med_frame3,
                frame_SNR,
                past_frames[current_frame - leng_tf : current_frame],
                mask2,
                bf,
                fft_object_b,
                fft_object_c,
        ...
        if update_baseline:
            ...
            if distri_update:
                ...
                nu2 = nu % Lu1
                med_frame3_temp[
                    :, nu2 * px_update : (nu2 + 1) * px_update, nu1 : nu1 + 1
                ] = median_calculation(
                    video_tf_past_fix[
                        :, nu2 * px_update : (nu2 + 1) * px_update, nu1 : nu1 + 1
                    ],
                ...
                if (
                    nu == Lu - 1
                ):  # update the baseline, noise, and buffer of recent frames
                    med_frame3 = med_frame3_temp.copy()
                    (video_tf_past_fix, video_tf_past) = (
                        video_tf_past,
                        video_tf_past_fix,
                    )
        ...
        segs = separate_neuron_online(
            frame_prob, pmaps_b, thresh_pmap_float, minArea, avgArea, useWT
        )
        ...
        if ((t - 0 - t_merge) == merge_every) or (t == nframesf - 1):
            if uniques.size:
                groupedneurons, times_groupedneurons = group_neurons(
                    uniques, thresh_COM, thresh_mask, dims, times_uniques, useMP=useMP
                )
        ...
        if ((t - 2 - t_merge) == merge_every) or (t == nframesf - 1):
            if uniques.size:
                ...
                if masks_add.size:
                    ...
                    # new binary masks
                    Masksb_add = [
                        (x >= x.max() * thresh_mask).astype("float") for x in masks_add
                    ]
                    # areas of new masks
        ...
        if (t + 1 - t_merge) != merge_every or t == (nframesf - 1):
            ...
            if t < (nframesf - 1):
                ...
                inactive = np.logical_and(
                    active_old_previous, np.logical_not(active_old)
                ).nonzero()[0]
            else:  # last frame
