...
from models.shallow_unet.shallow_unet_original_imp.suns.Network.data_gen import (
    data_gen_list,
)
...
def train_CNN(
    ...
    for Exp_ID in list_Exp_ID_train:
        ...
        list_train_imgs.append(
            np.array(
                h5_img["network_input"][
                    start_frame : train_every * num_train_per : train_every
                ]
            )
        )
        h5_img.close()
        h5_mask = h5py.File(os.path.join(dir_mask, Exp_ID + ".h5"), "r")
        list_train_masks.append(
            np.array(
    ...
    if list_Exp_ID_val is not None:
        ...
        for Exp_ID in list_Exp_ID_val:
            ...
            list_val_imgs.append(
                np.array(
                    h5_img["network_input"][
                        start_frame : val_every * num_val_per : val_every
                    ]
                )
            )
            h5_img.close()
            h5_mask = h5py.File(os.path.join(dir_mask, Exp_ID + ".h5"), "r")
            list_val_masks.append(
                np.array(
                    h5_mask["temporal_masks"][
                        start_frame : val_every * num_val_per : val_every
                    ]
