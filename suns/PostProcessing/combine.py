...
def segs_results(segs: list):
    ...
    probmapID = np.hstack(
        [ind * np.ones(x[1].size, dtype="uint32") for (ind, x) in enumerate(segs)]
    )
    return totalmasks, neuronstate, COMs, areas, probmapID


def unique_neurons2_simp(
    totalmasks: sparse.csr_matrix,
    neuronstate: np.array,
    COMs: np.array,
    areas: np.array,
    probmapID: np.array,
    minArea=0,
    ...
    if maxN > 0:  # False: #
        ...
        times = [
            probmapID[comb[ii].toarray().astype("bool").squeeze()] for ii in range(cnt)
        ]
...
def group_neurons(
    ...
    uniques_thresh = sparse.vstack(
        [x >= x.max() * thresh_mask for x in uniques]
    )  # binary masks
    COMs = np.zeros((N, 2))
    # Calculate COM
    for nn in range(N):
    ...
    uniquetimes = [
        np.hstack([times[ind] for ind in comb[ii].toarray().nonzero()[1]])
        for ii in range(cnt)
    ]
    return uniqueout, uniquetimes


def piece_neurons_IOU(
    neuronmasks: sparse.csr_matrix, thresh_mask, thresh_IOU, times: list
):
    ...
    tempmasks = sparse.vstack([x >= x.max() * thresh_mask for x in neuronmasks]).astype(
        "float"
    )  # binary masks
    # Calculate IoU
    ...
    area_u = a1 + a2 - area_i
    IOU = (
        np.triu(area_i) / area_u
    )  # IOU matrix is semmetric, so keeping a half is sufficient

    (x, y) = (IOU >= thresh_IOU).nonzero()  # indices of neuron pairs with high IOU
    belongs = np.arange(
        N
    )  # the indices of neurons merged to. Initially they are just themselves.
    for xi, yi in zip(x, y):
        ...
        if xto != yto:  # merge xto and yto
            ...
            move = (
                belongs == addfrom
            )  # merge all neurons previously merged to this neuron
            belongs[move] = addto  # merge larger index to smaller index
            uniquetimes[addto] = np.hstack(
                [uniquetimes[addfrom], uniquetimes[addto]]
            )  # merge active time
...
def piece_neurons_consume(
    neuronmasks: sparse.csr_matrix, avgArea, thresh_mask, thresh_consume, times: list
):
    ...
    (x, y) = (
        consume >= thresh_consume
    ).nonzero()  # indices of neuron pairs with consume
    for xi, yi in zip(x, y):
        area_x = area[xi]
        area_y = area[yi]
        if (
            max(area_x, area_y) > avgArea
        ):  # If the larger neuron is larger than avgArea, throw it
            if area_y > area_x:
                belongs[yi] = throw
    ...
    comb = sparse.csr_matrix(
        (np.ones(keep.size), (belongs[keep], np.arange(N)[keep])), (N, N)
    )
