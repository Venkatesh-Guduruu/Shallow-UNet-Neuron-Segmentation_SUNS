...
def complete_segment(
    pmaps: np.ndarray, Params: dict, useMP=True, useWT=False, display=False, p=None
):
    ...
    if useMP:
        segs = p.starmap(
            separate_neuron,
            [(frame, thresh_pmap, minArea, avgArea, useWT) for frame in pmaps],
            chunksize=1,
        )
    else:
        segs = [
    ...
    if num_neurons == 0:
        ...
        groupedneurons, times_groupedneurons = group_neurons(
            uniques, thresh_COM, thresh_mask, (dims[1], dims[2]), times_uniques
        )
        end_COM = time.time()
        # if display:
        #     print('{:25s}: Used {:9.6f} s, {:9.6f} ms/frame, '\
        ...
        start = time.time()
        Masks_2, times_cons = refine_seperate(
            masks_final_2, times_final, cons, thresh_mask
        )
        end_all = time.time()
        # if display:
        #     print('{:25s}: Used {:9.6f} s, {:9.6f} ms/frame, '\
...
# %%
def optimize_combine_1(
    uniques: sparse.csr_matrix,
    times_uniques: list,
    dims: tuple,
    Params: dict,
    filename_GT: str,
    ...
    groupedneurons, times_groupedneurons = group_neurons(
        uniques, thresh_COM, thresh_mask, (dims[1], dims[2]), times_uniques, useMP=False
    )
    # Merge neurons with high IoU.
    piecedneurons_1, times_piecedneurons_1 = piece_neurons_IOU(
        groupedneurons, thresh_mask, thresh_IOU, times_groupedneurons
        ...
        if useMP:
            ...
            for j1, thresh_COM in enumerate(list_thresh_COM):
                for j2, thresh_IOU in enumerate(list_thresh_IOU):
                    (Recal_k, Precision_k, F1_k) = optimize_combine_1(
                        uniques,
                        times_uniques,
                        dims,
                        {
