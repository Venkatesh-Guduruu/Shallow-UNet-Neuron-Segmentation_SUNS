...
def refineNeuron_WT(img, minArea, avgArea):
    ...
    tempstate = (
        nlabels <= 2
    )  # False if watershed segmentes the mask into multiple pieces.
    ...
    for k in range(1, nlabels):
        ...
        if (minArea < area_k <= avgArea) or (area_k > avgArea and tempstate):
            ...
            neuronSegment = labels == k
            tempxl, tempyl, centsl, _, areal = refineNeuron_WT(
                neuronSegment.astype("uint8"), minArea, avgArea
            )
            dcnt = len(centsl)
            if dcnt:
    ...
    if num > 0:
        ...
        for k, k_area in enumerate(areas):
            if k_area > avgArea:
                ...
                BW1 = sparse.coo_matrix(
                    (np.ones(k_area, dtype="uint8"), (rows - ymin, cols - xmin)),
                    shape=(ysize, xsize),
                ).toarray()

                # Apply watershed to each mask, and create a series of possibly smaller masks
                tempx1, tempy1, tempcent, tempstate, temparea = refineNeuron_WT(
                    BW1, minArea, avgArea
                )
                dcnt = len(tempcent)
                if dcnt > 0:
                    ...
                    masks_new = masks_new + [
                        sparse.csr_matrix(
                            (
                                np.ones(x1.size) * (1 / 4 + 3 / 4 * tempstate),
                                (
                                    np.zeros(x1.size, dtype="int"),
                                    (y1 + ymin) * dims[1] + (x1 + xmin),
                                ),
                            ),
                            shape=(1, dims[0] * dims[1]),
                        )
                        for (x1, y1) in zip(tempx1, tempy1)
                    ]
...
def separate_neuron(img: np.array, thresh_pmap=None, minArea=0, avgArea=0, useWT=False):
    ...
    if (
        nlabels > 0
    ):  # nlabel is the number of connected components. If nlabel=0, no active pixel is found
        for k in range(1, nlabels):  # for all connected components
            current_stat = stats[k]
            k_area = current_stat[4]
            if (
                k_area > minArea
            ):  # Only keep the connected regions with area larger than minArea
                # BW = (labels == k)
                # Crop a small rectangle containing mask k
                ...
                if (
                    useWT and k_area > avgArea
                ):  # If useWT==True and the mask area is larger than avgArea,
                    ...
                    if current_stat[0] == 0:
                        xmin = -1
                        BW1 = np.pad(
                            BW1, ((0, 0), (1, 0)), "constant", constant_values=(0, 0)
                        )
                    if current_stat[0] + current_stat[2] == dims[1]:
                        BW1 = np.pad(
                            BW1, ((0, 0), (0, 1)), "constant", constant_values=(0, 0)
                        )
                    if current_stat[1] == 0:
                        ymin = -1
                        BW1 = np.pad(
                            BW1, ((1, 0), (0, 0)), "constant", constant_values=(0, 0)
                        )
                    if current_stat[1] + current_stat[3] == dims[0]:
                        BW1 = np.pad(
                            BW1, ((0, 1), (0, 0)), "constant", constant_values=(0, 0)
                        )
                    # Apply watershed to each mask, and create a series of possibly smaller masks
                    tempx1, tempy1, tempcent, tempstate, temparea = refineNeuron_WT(
                        BW1.astype("uint8"), minArea, avgArea
                    )

                    dcnt = len(
                        tempcent
                    )  # Number of segmented pieces obtained by watershed
                    if dcnt > 0:
                        # convert a 2D coordiate into a 1D index
                        tempind = [
                            (y1 + ymin) * dims[1] + (x1 + xmin)
    ...
    if neuron_cnt == 0:  # does not find any neuron
        ...
        ind_row = np.hstack(
            [j * tj for (j, tj) in enumerate(temp)]
        )  # corresponding index of segmented neurons
        # weighted by 1/4 for masks segmented after watershed
        vals = np.hstack(
            [(1 / 4 + 3 / 4 * sj) * tj for (sj, tj) in zip(neuronstate, temp)]
        )
        # create a sparse matrix with "neuron_cnt" rows to represent the "neuron_cnt" segmented neurons
        masks = sparse.csr_matrix(
            (vals, (ind_row, ind_col)), shape=(neuron_cnt, dims[0] * dims[1])
        )
        # convert "neuronstate", "cents", and "areas" from list to numpy.array
