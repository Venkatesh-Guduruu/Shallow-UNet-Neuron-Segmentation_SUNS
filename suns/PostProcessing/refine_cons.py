...
def refine_seperate_multi(
    ...
    for k1, cons in enumerate(list_cons):
        if cons > 1:
            ...
            if np.any(have_cons):
                ...
                (Recall_k[k1], Precision_k[k1], F1_k[k1]) = GetPerformance_Jaccard_2(
                    GTMasks_2, Masks_2, ThreshJ
                )
            else:
    ...
    if display:
        ind = F1_k.argmax()
        print(
            "Recall={:0.6f}, Precision={:0.6f}, F1={:0.6f}, cons={}.".format(
