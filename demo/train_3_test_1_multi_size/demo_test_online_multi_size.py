...
if __name__ == "__main__":
    ...
    list_rate_hz = [10] * len(
        list_Exp_ID
    )  # frame rate of all the videos. Close frame rates are preferred.
    list_Mag = [6 / 8] * len(
        list_Exp_ID
    ...
    for CV in list_CV:
        ...
        if useTF:
            ...
            x = (
                np.arange(
                    np.round(-peak * rate_hz / fs_template),
                    np.round(length - peak * rate_hz / fs_template),
                    1,
                )
                / rate_hz
            )
        ...
        filename_params_post = os.path.join(
            dir_params, "Optimization_Info_{}.mat".format(CV)
        )
        # If you used cross_validation == 'use_all' in training, you need to change the "CV" in "format(CV)"
        # to the number of tranining videos used.
