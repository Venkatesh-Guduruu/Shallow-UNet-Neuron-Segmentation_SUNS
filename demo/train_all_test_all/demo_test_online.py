...
if __name__ == "__main__":
    ...
    filename_TF_template = (
        "../YST_spike_tempolate.h5"  # file name of the temporal filter kernel
    )
    ...
    dir_parent = os.path.join(
        dir_video, "noSF use_all"
    )  # folder to save all the processed data for test videos
    dir_parent_train = os.path.join(
        dir_video_train, "noSF use_all"
    )  # folder to save all the processed data for train videos
    dir_output = os.path.join(
        dir_parent, "output_masks online"
    )  # folder to save the segmented masks and the performance scores
    dir_params = os.path.join(
        dir_parent_train, "output_masks"
    )  # folder of the optimized hyper-parameters
    weights_path = os.path.join(
        dir_parent_train, "Weights"
    )  # folder of the trained CNN
    ...
    for CV in list_CV:
        ...
        print("Video ", Exp_ID)
        filename_video = os.path.join(
            dir_video, Exp_ID + ".h5"
        )  # The path of the file of the input video.
        filename_CNN = os.path.join(
            weights_path, "Model_CV{}.h5".format(nvideo_train)
        ...
        filename_params_post = os.path.join(
            dir_params, "Optimization_Info_{}.mat".format(nvideo_train)
        )
        # If you used cross_validation == 'use_all' in training, you need to change the "CV" in "format(CV)"
        # to the number of tranining videos used.
