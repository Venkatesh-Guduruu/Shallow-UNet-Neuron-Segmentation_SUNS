...
from suns.Online.train_params_online import (
    parameter_optimization_cross_validation_online,
)
...
if __name__ == "__main__":
    ...
    list_thred_ratio = [
        thred_std
    ]  # A list of SNR threshold used to determine when neurons are active.
    ...
    dir_parent = os.path.join(
        dir_video, "noSF online"
    )  # folder to save all the processed data
    dir_network_input = os.path.join(
        dir_parent, "network_input"
    )  # folder of the SNR videos
    dir_mask = os.path.join(
        dir_parent, "temporal_masks({})".format(thred_std)
    )  # foldr to save the temporal masks
    weights_path = os.path.join(dir_parent, "Weights")  # folder to save the trained CNN
    training_output_path = os.path.join(
        dir_parent, "training output"
    )  # folder to save the loss functions during training
    dir_output = os.path.join(
        dir_parent, "output_masks"
    )  # folder to save the optimized hyper-parameters
    dir_temp = os.path.join(
        dir_parent, "temp"
    )  # temporary folder to save the F1 with various hyper-parameters
    ...
    num_total = (
        nframes - Poisson_filt.size + 1
    )  # number of frames used for CNN training.
    ...
    for Exp_ID in list_Exp_ID:  #
        # %% Pre-process video
        video_input, _ = preprocess_video_online(
            dir_video,
            Exp_ID,
            Params_pre,
            frames_init,
    ...
    for CV in list_CV:
        ...
        f = h5py.File(
            os.path.join(training_output_path, "training_output_CV{}.h5".format(CV)),
            "w",
        )
