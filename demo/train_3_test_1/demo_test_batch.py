...
from models.shallow_unet.shallow_unet_original_imp.suns.PostProcessing.evaluate import (
    GetPerformance_Jaccard_2,
)
from models.shallow_unet.shallow_unet_original_imp.suns.run_suns import suns_batch
...
if __name__ == "__main__":
    ...
    dir_video = "models/shallow_unet/shallow_unet_original_imp/demo/data"
    ...
    Mag = (
        6 / 8
    )  # spatial magnification compared to ABO videos (0.785 um/pixel). # Mag = 0.785 / pixel_size

    # %% set pre-processing parameters
    gauss_filt_size = (
        50 * Mag
    )  # standard deviation of the spatial Gaussian filter in pixels
    num_median_approx = 1000  # number of frames used to caluclate median and median-based standard deviation
    filename_TF_template = (
        "models/shallow_unet/shallow_unet_original_imp/demo/YST_spike_tempolate.h5"
    )
    # File name storing the temporal filter kernel
    ...
    prealloc = (
        True  # True if pre-allocate memory space for large variables in pre-processing.
    )
    ...
    dir_parent = os.path.join(
        dir_video, "noSF"
    )  # folder to save all the processed data
    dir_output = os.path.join(
        dir_parent, "output_masks"
    )  # folder to save the segmented masks and the performance scores
    dir_params = os.path.join(
        dir_parent, "output_masks"
    )  # folder of the optimized hyper-parameters
    ...
    for CV in list_CV:
        ...
        print("Video ", Exp_ID)
        filename_CNN = os.path.join(
            weights_path, "Model_CV.h5".format(CV)
        )  # The path of the CNN model.
        ...
        Optimization_Info = loadmat(
            os.path.join(dir_params, "Optimization_Info.mat".format(CV))
        )
        # If you used cross_validation == 'use_all' in training, you need to change the "CV" in "format(CV)"
        # to the number of tranining videos used.
        ...
        savemat(
            os.path.join(dir_output, "Output_Masks_{}.mat".format(Exp_ID)),
            {"Masks": Masks, "times_active": times_active},
            do_compression=True,
        )
        ...
        (Recall, Precision, F1) = GetPerformance_Jaccard_2(
            GTMasks_2, Masks_2, ThreshJ=0.5
        )
        ...
        Info_dict = {
            ...
            "list_F1": list_F1,
            "list_time": list_time,
            "list_time_frame": list_time_frame,
        }
